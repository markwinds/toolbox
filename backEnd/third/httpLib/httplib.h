...
inline unsigned char to_lower(int c) {
    const static unsigned char table[256] = {
        ...
        180, 181, 182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 224, 225, 226, 227, 228, 229,
        230, 231, 232, 233, 234, 235, 236, 237, 238, 239, 240, 241, 242, 243, 244, 245, 246, 215,
...
using Headers = std::unordered_multimap<std::string,
                                        std::string,
...
class ClientImpl {
  ...
  private:
    ...
    get_multipart_content_provider(const std::string&                    boundary,
                                   const MultipartFormDataItems&         items,
...
inline void default_socket_options(socket_t sock) {
    ...
    setsockopt(
        sock, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, reinterpret_cast<const char*>(&opt), sizeof(opt));
#else
#ifdef SO_REUSEPORT
...
#ifdef _WIN32
            setsockopt(
                sock, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&opt), sizeof(opt));
#else
            setsockopt(
                sock, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const void*>(&opt), sizeof(opt));
#endif
        }
...
#ifdef _WIN32
            setsockopt(
                sock, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<const char*>(&opt), sizeof(opt));
#else
            setsockopt(
                sock, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<const void*>(&opt), sizeof(opt));
#endif
        }
    ...
    auto sock = create_socket(
        ...
        [&](socket_t sock2, struct addrinfo& ai, bool& quit) -> bool {
...

            {
                ...
                setsockopt(
                    sock2, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const void*>(&tv), sizeof(tv));
#endif
            }
            {
                ...
                setsockopt(
                    sock2, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const void*>(&tv), sizeof(tv));
#endif
            }
...
template <typename T, typename U>
bool prepare_content_receiver(
    T& x, int& status, ContentReceiverWithProgress receiver, bool decompress, U callback) {
    if (decompress) {
...
inline bool parse_range_header(const std::string& s, Ranges& ranges) try {
    ...
    auto is_valid = [](const std::string& str) {
        return std::all_of(
            str.cbegin(), str.cend(), [](unsigned char c) { return std::isdigit(c); });
    };
...
make_digest_authentication_header(const Request&                            req,
...
inline bool PathParamsMatcher::match(Request& request) const {
    ...
    for (size_t i = 0; i < static_fragments_.size(); ++i) {
        ...
        // comparison
        if (std::strncmp(
                request.path.c_str() + starting_pos, fragment.c_str(), fragment.length()) != 0) {
            return false;
        }
...
inline bool Server::write_content_with_provider(Stream&            strm,
    ...
    if (res.content_length_ > 0) {
        if (req.ranges.empty()) {
            return detail::write_content(
                strm, res.content_provider_, 0, res.content_length_, is_shutting_down);
            ...
            return detail::write_multipart_ranges_data(
                strm, req, res, boundary, content_type, res.content_length_, is_shutting_down);
        }
    } else {
...

    {
        ...
        while (svr_sock_ != INVALID_SOCKET) {
...

            {
                ...
                setsockopt(
                    sock, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const void*>(&tv), sizeof(tv));
#endif
            }
            {
                ...
                setsockopt(
                    sock, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const void*>(&tv), sizeof(tv));
#endif
            }
...
inline bool
Server::dispatch_request_for_content_reader(Request&                        req,
                                            Response&                       res,
                                            ContentReader                   content_reader,
                                            const HandlersForContentReader& handlers) const {
    for (const auto& x : handlers) {
        ...
        if (!username.empty() && !password.empty()) {
            ...
            if (detail::parse_www_authenticate(res, auth, is_proxy)) {
                ...
                new_req.headers.insert(
                    detail::make_digest_authentication_header(req,
                                                              auth,
...
ClientImpl::write_content_with_provider(Stream& strm, const Request& req, Error& error) const {
    ...
    if (req.is_chunked_content_provider_) {
        ...
        return detail::write_content(
            strm, req.content_provider_, 0, req.content_length_, is_shutting_down, error);
    }
}
    ...
    if (compress_ && !content_provider_without_length) {
        ...
        if (content_provider) {
            ...
            if (!compressor.compress(
                    body, content_length, true, [&](const char* data, size_t data_len) {
...
inline Result
ClientImpl::send_with_content_provider(const std::string&           method,
                                       const std::string&           path,
                                       const Headers&               headers,
                                       const char*                  body,
                                       size_t                       content_length,
...
inline Result ClientImpl::Post(const std::string& path,
    ...
    return send_with_content_provider(
        "POST", path, headers, body, content_length, nullptr, nullptr, content_type, nullptr);
}
...
inline Result ClientImpl::Post(const std::string& path,
                               ...
                               Progress           progress) {
    return send_with_content_provider(
        "POST", path, headers, body, content_length, nullptr, nullptr, content_type, progress);
}
...
inline Result ClientImpl::Post(const std::string& path,
                               ...
                               const std::string& content_type) {
    return send_with_content_provider(
        "POST", path, headers, body.data(), body.size(), nullptr, nullptr, content_type, nullptr);
}
...
inline Result ClientImpl::Post(const std::string& path,
                               ...
                               Progress           progress) {
    return send_with_content_provider(
        "POST", path, headers, body.data(), body.size(), nullptr, nullptr, content_type, progress);
}
...
inline Result ClientImpl::Put(const std::string& path,
                              ...
                              const std::string& content_type) {
    return send_with_content_provider(
        "PUT", path, headers, body, content_length, nullptr, nullptr, content_type, nullptr);
}
...
inline Result ClientImpl::Put(const std::string& path,
                              ...
                              Progress           progress) {
    return send_with_content_provider(
        "PUT", path, headers, body, content_length, nullptr, nullptr, content_type, progress);
}
...
inline Result ClientImpl::Put(const std::string& path,
                              ...
                              const std::string& content_type) {
    return send_with_content_provider(
        "PUT", path, headers, body.data(), body.size(), nullptr, nullptr, content_type, nullptr);
}
...
inline Result ClientImpl::Put(const std::string& path,
                              ...
                              Progress           progress) {
    return send_with_content_provider(
        "PUT", path, headers, body.data(), body.size(), nullptr, nullptr, content_type, progress);
}
...
inline Result ClientImpl::Patch(const std::string& path,
                                ...
                                Progress           progress) {
    return send_with_content_provider(
        "PATCH", path, headers, body, content_length, nullptr, nullptr, content_type, progress);
}
...
inline Result ClientImpl::Patch(const std::string& path,
                                ...
                                Progress           progress) {
    return send_with_content_provider(
        "PATCH", path, headers, body.data(), body.size(), nullptr, nullptr, content_type, progress);
}
    ...
    return process_server_socket_core(svr_sock,
                                      ...
                                      [&](bool close_connection, bool& connection_closed) {
                                          SSLSocketStream strm(sock,
                                                               ...
                                                               write_timeout_usec);
                                          return callback(
                                              strm, close_connection, connection_closed);
                                      });
}
...
inline bool process_client_socket_ssl(SSL*     ssl,
    ...
    SSLSocketStream strm(
        sock, ssl, read_timeout_sec, read_timeout_usec, write_timeout_sec, write_timeout_usec);
    return callback(strm);
}
...
inline Result Client::Get(const std::string& path,
                          ...
                          Progress           progress) {
    return cli_->Get(
        path, std::move(response_handler), std::move(content_receiver), std::move(progress));
}
inline Result Client::Get(const std::string& path,
                          const Headers&     headers,
                          ResponseHandler    response_handler,
